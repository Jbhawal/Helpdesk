<?php
    ...
    if(isset($_POST['submitBtn'])){
        ...
        $currentFilterOnSubmit = $_POST["currentFilter"] ?? 'All'; 
        $dbo->query("UPDATE complaints SET admrem='$adminRemarks', status='$newStatus' WHERE compid='$complaintIdToUpdate'");
        echo "<script>
    ...
    if(isset($_GET['e'])){
        $complaintId = $_GET['e'];
        $list = $dbo->query("SELECT COMPID, CTYPE, SUB, DESCR, UPLOADEDFILE, STATUS, ADMREM FROM complaints WHERE compid = '$complaintId'");
                    ...
                    <form action="" method="post" class="complaint-form">
                        ...
                        <div class="clist-container">
                            <table class="clist-table">
                                ...
                                <tbody>
                                    <?php
                                    $whereClause = "WHERE status IN('Forwarded to Admin', 'Closed', 'Rejected')"; 
                                    $noComplaintsMessage = "No complaints found 'Forwarded to Admin', 'Closed', or 'Rejected'.";
